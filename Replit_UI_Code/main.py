...
def prepare_input(credit_score, location, gender, age, tenure, balance, num_products, has_credit_card, is_active_memeber, estimated_salary):
...
def explain_prediction(probability, input_dict, surname):
  ...
  Your machine learning model has predicted that a customer named {surname} has a probability of {round(probability * 100, 1)}% of churning, based on the information provided below.
...
-If the customer has over a 40% risk of churning, generate a short summary of around 3 sentences explaning why they are at a risk of churning.
- If the customer has less than 40% risk of churning, generate a short summary of around 3 senetences explaning why they might not be at a risk of churning.
- Your explanation should be based on the customer's information, the summary statistics of churned and non-churned customers, and the feature importances provided.
  Dont mention the probability of churning or the machine learning model, or say anything like "Based on the machine learning model's prediction and top10 most important features", just explain the prediction. """
  
  raw_response = client.chat.completions.create(
      model="llama-3.2-90b-text-preview",
      messages=[
          {"role": "system", "content": "You are a helpful assistant that explains customer churn predictions."},
...
def generate_email(probability, input_dict, explanation, surname):
  prompt = f"""You are a manager at Euro Bank. You are responsible for ensuring customers stay with the bank and are incentivized with various offers.
  ...
  Say at the end this mail is from Manager at Euro Bank.
  """
  raw_response = client.chat.completions.create(
    model = "gemma2-9b-it",
